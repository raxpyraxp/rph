...
| group             | `big-models`                                                                    | A group name. Only one process from the group can be active. If some process gets a request - all others in this group are terminating.                                                                                                                                                                                                   |
| ignore_http_codes | `502`                                                                           | An array that holds a list of http codes that will be ignored. For example, some services can bring the server up, but it's not ready yet, so it replies with "Not ready" code like 502. This array will contain codes that would still mean that the server is unavailable and we have to keep trying before replying to our real client |
